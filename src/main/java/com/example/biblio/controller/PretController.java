...
public class PretController {
    ...
    public String demanderPret(
            ...
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDateTime datePret,
            HttpSession session,
        ...
        try {
            ...
            if (datePret != null && datePret.isBefore(LocalDateTime.now())) {
                throw new RuntimeException("La date de prêt doit être maintenant ou une date future");
            }

            Pret pret = pretService.demanderPret(titre, auteur, adherent, datePret);
